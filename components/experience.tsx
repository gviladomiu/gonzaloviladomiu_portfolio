  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        {/* Experiences as a vertical record list */}
        <div className="space-y-16 md:space-y-24">
          {experience.map((exp, i) => (
            <Reveal key={exp.company}>
              <article className="grid md:grid-cols-12 gap-8 group">
                {/* Left: meta */}
                <div className="md:col-span-4 md:sticky md:top-24 self-start">
                  ...
                  <div className="mt-6 flex flex-wrap gap-1.5">
                    {exp.stack.map((s) => (
                      <span
                        key={s}
                        className="font-mono text-[11px] tracking-snug text-ink-500 bg-surface border border-ink-200 px-2.5 py-1"
                      >
                        {s}
                      </span>
                    ))}
                ...
                <div className="md:col-span-7 md:col-start-6">
                  <p className="text-base md:text-lg text-ink-700 leading-relaxed mb-8 max-w-xl border-l-2 border-ink-200 pl-5 italic">
                    {exp.summary}
                  </p>
                  ...
                  <ul className="space-y-0">
                    {exp.highlights.map((h, hi) => (
                      <li
                        key={hi}
                        className="flex gap-5 py-4 border-b border-ink-200 last:border-b-0 text-[14px] md:text-[15px] text-ink-700 leading-relaxed"
                      >
